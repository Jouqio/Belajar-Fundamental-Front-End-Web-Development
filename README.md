...
```
belajar-frontend-web/
├── Teknik-Layouting-CSS-Grid/
│   └── 01-15 modul + contoh interaktif
├── Web-Component/
│   └── 01-20 modul + contoh interaktif
├── Package-Manager-untuk-Aplikasi-Web/
│   └── 01-12 modul + contoh CLI
├── Proyek-Awal/
│   ├── 01-Tips-Submission-Proyek-Pertama/
│   └── 02-Submission-Membangun-Notes-App/
├── notes-app/
│   └── Aplikasi Notes App siap jalan
├── HTML-Form-Lanjutan/
├── Asynchronous-JavaScript-Request/
│   └── 01-24 modul + mock API server + studi kasus
...
1. **Clone repo ini**
   ```bash
   git clone https://github.com/Jouqio/belajar-frontend-web.git
   cd belajar-frontend-web
   ```
2. **Masuk ke folder modul** yang ingin dipelajari, baca `README.md`-nya terlebih dahulu.
3. **Buka contoh kode** (biasanya di folder `contoh/`) lewat browser — disarankan pakai **Live Server** (VS Code) agar semua fitur (termasuk ES Module & fetch API) berjalan normal.
4. Untuk modul yang butuh server lokal (misalnya *Asynchronous JavaScript Request*), jalankan `node server.js` sesuai instruksi di README modul tersebut.
...
- **CSS Grid** — layouting halaman modern tanpa framework
- **Web Component** — Custom Element, Shadow DOM, `<template>`
- **npm** — package management, semantic versioning, scripting
- **XMLHttpRequest & Fetch API** — komunikasi asynchronous dengan server
- **Module Bundler** — optimasi & penggabungan kode untuk produksi
- **Clean Code** — struktur folder rapi, penamaan jelas, kode mudah dibaca
...
- [x] Proyek Awal — Notes App
- [x] HTML Form Lanjutan
- [x] Asynchronous JavaScript Request
- [x] Module Bundler
...
**⭐ Kalau repo ini membantu perjalanan belajarmu, jangan lupa kasih star!**
